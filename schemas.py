        ...
        with connection:
            with connection.cursor() as cursor:
                ...
                if not user_id:
                    ...
                    insert_user_data = 'INSERT INTO user (name, email, phone) VALUE (%s, %s, %s) RETURNING id;'
    ...
    try:
        ...
        with connection:
            with connection.cursor() as cursor:
                ...
                images.pass_id=id AND passes.id=%s''', (pass_id,))
                columns = [column[0] for column in cursor.description]
    ...
    try:
        ...
        with connection:
            with connection.cursor() as cursor:
                ...
                passes.latitude, passes.longitude, passes.height FROM passes, user WHERE
                passes.user_id=user.id AND users.email=%s''', (user_email,))
                columns = [column[0] for column in cursor.description]
