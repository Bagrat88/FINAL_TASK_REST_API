...
from schemas import Response, AddPass, get_pass, PassOptional, pass_details, ResponseUpdate, ResponseUpdate, get_user_passes
...
async def validation_exception_handler(request, exc):
    ...
    return JSONResponse(status_code=status.HTTP_400_BAD_REQUEST, content=jsonable_encoder(({'status': 400, 'message': 'Неправильно введены данные', 'errors': response_message})))
...
@app.patch('/update/{pass_id}', response_model=AddPass)
    ...
    try:
        ...
        if stored_data:
            if stored_data.get('status') == 'new':
                ...
                update_pass = stored_model.copy(update=update_data)
                update_pass.update_pass(pass_id)
...
@app.get('/passes/user/{user_email}')
